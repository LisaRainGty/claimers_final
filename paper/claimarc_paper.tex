...
If RACL works as intended, it should reshape the retrieval embedding so that
neighbors in the index share the queried attribute and the perceived-risk label,
which is exactly what the RKC vote (Section~\ref{sec:exp:xdom}) and the gradient-free
library adaptation read off. We test this on the test embeddings
$\mathbf{g}_{p,a}$ with two attribute-conditioned quantities: the mean cosine to
same-attribute \emph{same-label} neighbors (tightness, higher is better) and the
attribute-conditioned retrieval quality (mAP@10, the precision of the
ten nearest neighbors at sharing the query's attribute). Both compare the
canonical against the same network trained \emph{without} the contrastive
objective, so the contrast is the only difference. Table~\ref{tab:geom} reports
them. RACL tightens same-label neighborhoods from 0.628 to 0.764 and lifts the
attribute-retrieval mAP@10 from 0.296 to 0.325; the contrast organizes the local
neighborhood so that retrieval returns same-attribute, same-label exemplars, the
structure the vote depends on. The UMAP projection in Figure~\ref{fig:umap} shows
the label structure by eye: under the full model the misleading pairs condense
into a detached region, whereas removing RACL leaves the two classes interleaved.
This is the geometric counterpart of the AP gain in
Section~\ref{sec:exp:main} and of why the frozen-encoder vote becomes useful only
after RACL (Section~\ref{sec:exp:xdom}).
...
\begin{tabular}{lcc}
\toprule
\textbf{Variant} & \textbf{Same-label tight} $\uparrow$ & \textbf{Attr.\ mAP@10} $\uparrow$ \\
\midrule
CLAIMARC w/o RACL        & 0.628 & 0.296 \\
\textbf{CLAIMARC (full)} & \best{0.764} & \best{0.325} \\
\bottomrule
\end{tabular}
\caption{Attribute-conditioned geometry on the test set (new canonical, single
seed). The class-balanced retrieval-augmented contrast tightens same-label
neighborhoods and improves attribute-conditioned retrieval quality (mAP@10)
relative to the same model trained without the contrastive objective.}
\label{tab:geom}
\end{table}
...
\caption{UMAP of the retrieval embedding $\mathbf{g}_{p,a}$ (test), colored by
perceived-risk label. The full model (left) isolates the misleading pairs into
a detached cluster; removing RACL (middle) collapses the separation.}
\label{fig:umap}
\end{figure}
...
RACL produces the intended geometry, tightening same-attribute same-label
neighborhoods from 0.628 to 0.764 and lifting attribute-conditioned retrieval
quality (mAP@10) from 0.296 to 0.325; this is the geometric structure the
retrieval vote and library adaptation exploit. On RQ3, CLAIMARC transfers best of all learned systems, leading the
...
look alike yet carry different labels; the geometry analysis verifies that this
tightens same-label neighborhoods and improves attribute-conditioned retrieval,
the structure the retrieval vote relies on. At the
